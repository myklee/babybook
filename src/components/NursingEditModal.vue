...
function validateSession(): Array<{ field: string; message: string }> {
  ...
  if (
    ...
    const startDateTime = getDateTimeFromInputs(
      startDate.value,
      startTime.value
    );
    const endDateTime = getDateTimeFromInputs(endDate.value, endTime.value);
    ...
    if (startDateTime && endDateTime) {
      ...
      if (endDateTime > startDateTime) {
        ...
        if (durationMs < 30000) {
          errors.push({
            field: "duration",
            message: "Session duration must be at least 30 seconds",
          });
        }
        ...
        // Check for unreasonably short duration (less than 1 minute) with warning
          ...
          errors.push({
            field: "duration",
            message:
              "Session duration is very short (less than 1 minute). Please verify the times are correct.",
          });
        }
        ...
        // Check for very long duration (more than 4 hours) with warning
          ...
          if (durationHours > 24) {
            ...
            errors.push({
              field: "duration",
              message: `Session duration is ${
                Math.round(durationHours * 10) / 10
              } hours. Please verify this is correct.`,
            });
          }
        }
        ...
        if (startDay !== endDay && durationHours > 12) {
          errors.push({
            field: "duration",
            message:
              "Multi-day sessions longer than 12 hours should be split into separate sessions",
          });
        }
      }
      ...
      if (startDateTime < oneYearAgo) {
        errors.push({
          field: "start_date",
          message: "Start date cannot be more than 1 year ago",
        });
      }
      ...
      if (endDateTime < oneYearAgo) {
        errors.push({
          field: "end_date",
          message: "End date cannot be more than 1 year ago",
        });
      }
    }
  }
...
watch([startDate, endDate], () => {
  if (props.isOpen && editSession.value) {
    ...
    const dateErrors = currentErrors.filter(
      (error) =>
...
<template>
    ...
    <form @submit.prevent="handleSave" class="edit-form">
        ...
        <div class="time-inputs">
          <div class="form-group">
            ...
            <div v-if="startDateErrors.length > 0" class="field-errors">
              <div
                v-for="error in startDateErrors"
                :key="error.message"
                class="field-error"
              >
                {{ error.message }}
          ...
          <div class="form-group">
            ...
            <div v-if="startTimeErrors.length > 0" class="field-errors">
              <div
                v-for="error in startTimeErrors"
                :key="error.message"
                class="field-error"
              >
                {{ error.message }}
      ...
      <div class="form-section">
        <div
          ...
          :class="{ error: hasFieldError('duration') }"
        >
  ...
  >
    <p class="delete-confirmation-message">
      Are you sure you want to delete this nursing session? This action cannot
      be undone.
    </p>
    <div v-if="editSession" class="session-details">
      <p class="session-detail">
        <strong>Start:</strong>
        {{ format(new Date(editSession.start_time), "MMM d, yyyy h:mm a") }}
      </p>
      <p class="session-detail">
      ...
      <p class="session-detail">
        ...
        {{
          editSession.breast_used === "left"
            ? "Left"
            : editSession.breast_used === "right"
            ? "Right"
            : "Both"
        }}
      </p>
